...
/**
 * This function gets the value of a long term session attribute. These
 ...
 * get_order_number_for_status_check // this would prompt for order number and
 * store in current_order then jump to check_order_status - else: ->
 * check_order_status // retrieves and displays status for order number in
 * current_order } -> END ```
 * 
 * Note: If there is no long term attribute with the specified name in the
 * session then the specified Ink variable will be set to "".
 * 
 * 
 * @author rabidgremlin
 *
 */
public class GetLongTermAttributeFunction implements InkBotFunction
{
